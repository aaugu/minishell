...
/*   Updated: 2023/06/01 16:18:25 by aaugu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_new_env(t_data *data, int new_size);
 ...
 */
void	cmd_unset(t_data *d)
{
	int		i;
	int		res;
	int		count;

	if (ft_strs_len(d->cmd) <= 1)
	...
	else
	{
		...
		if (count > 0)
		{
			get_new_env(d, d->env_size - count);
			d->env_size -= count;
		}
	}
}
