...
/*   Updated: 2023/05/09 15:26:17 by lvogt            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sort_env_alph(char **env_copy, int count)
{
	...
	while (++i < count - 1)
	{
		...
		while (i < count)
		{
			if (ft_strncmp(env_copy[i], env_copy[i], 255) > 0)
			{
				temp = env_copy[i];
				env_copy[i] = env_copy[i];
				env_copy[i] = temp;
			}
			i++;
		}
...
int	ft_export_nbr(t_data *data)
{
	...
	while (data->cmd[i])
	{
		if (ft_check_gs(data->cmd[i]) == 0
			&& ft_digit_equal(data->cmd[i]) == 0)
			export_nbr++;
		else
		{
...
char	**ft_add(int line, int export_nbr, t_data *data)
{
	...
	while (data->cmd[++j])
	{
		...
		if (ft_check_gs(data->cmd[j]) == 0
			&& ft_digit_equal(data->cmd[j]) == 0)
		{
			if (exist >= 0)
			{
				free(export[exist]);
